...
{
    public class RecordImportedNotification<T> : INotification where T : IContentBase
    {
        ...
        public RecordImportedNotification(T item, ImportState state, object primaryKeyValue, Hashtable items)
        {
            Item = item;
            ImportState = state;
